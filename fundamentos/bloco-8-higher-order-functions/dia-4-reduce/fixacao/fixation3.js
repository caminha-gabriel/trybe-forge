...
const getBiggerNote = (acc, cur) => {
  if (cur.nota > acc.nota) {
    return cur
  } return acc
};

const createObj = (array) => array.map((obj) => ({
  name: obj.nome,
  materia: obj.materias.reduce(getBiggerNote).name,
  nota: obj.materias.reduce(getBiggerNote).nota,
}));

console.log(createObj(estudantes));